...
def page_is_current(body, entry):
    ...
    if block_ids(blocks[0]) != want:
        return False
    # An assignment can be RENAMED in Canvas without its id changing. The href
    # still resolves, but the visible link text on the page is whatever the
    # title was when the block was written. Comparing ids alone would call such
    # a page "current" and skip it, leaving the old section number on display.
    # entry["assignment_name"] is refreshed from live Canvas above, so compare
    # the labels too.
    for nm in (entry.get("assignment_name"), entry.get("practice_name")):
        if nm and html.escape(nm) not in blocks[0]:
            return False
    return True
